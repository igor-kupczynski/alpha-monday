...
- `db-up`: start local Postgres via `./scripts/db-up`.
- `db-down`: stop local Postgres via `./scripts/db-down`.
- `db-reset`: reset local Postgres volume via `./scripts/db-reset`.
...
## Notes
- The Makefile wraps scripts to keep behavior consistent across local development and CI.
